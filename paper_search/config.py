...
_ENV_LOADED = False
ENV_PREFIX = "PAPER_SEARCH_"
_LEGACY_PREFIX = "PAPER_SEARCH_MCP_"


def _candidate_env_files() -> list[Path]:
    explicit_path = os.getenv(f"{ENV_PREFIX}ENV_FILE", "").strip()
    if not explicit_path:
        explicit_path = os.getenv(f"{_LEGACY_PREFIX}ENV_FILE", "").strip()
    if explicit_path:
...
def get_env(name: str, default: Optional[str] = "") -> str:
    ...
    keys = [f"{ENV_PREFIX}{normalized}", f"{_LEGACY_PREFIX}{normalized}", normalized]
    for key in keys:
